...
{
	if (origin & RECT_ORIGIN_CENTER)
	{
		aabb->position = (struct vector3){rect.position.x - rect.scale.x * 0.5f, rect.position.y - rect.scale.y * 0.5f, 0.0f};
		aabb->scale = rect.scale;
	}
	else if (origin & RECT_ORIGIN_TOP_LEFT)
	{
		aabb->position = rect.position;
		aabb->scale = rect.scale;
	}
}
